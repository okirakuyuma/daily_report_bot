...
本プロジェクトでは `webapp/docs/` と `infra/docs/` にドキュメントを配置し、各ディレクトリには **template.md** が用意されています。新規ドキュメント作成時は必ず対応する `template.md` に従って記入してください。
...
> **重要**: 各ディレクトリの `template.md` がプロジェクト固有のフォーマットを定義しています。Skillsを使用する際も、これらのテンプレートに従った出力を行います。
...
## 出力場所
- webapp関連: `webapp/docs/prd/{feature-name}.md`
- infra関連: `infra/docs/prd/{feature-name}.md`

## テンプレート参照
**必ず対応するディレクトリの `template.md` を読み込んで、そのフォーマットに従って作成してください。**

- webapp: `webapp/docs/prd/template.md`
- infra: `infra/docs/prd/template.md`

## 作成プロセス

1. まず対応する `template.md` を Read ツールで読み込む
2. テンプレートの構造に従って内容を記入
3. 既存のPRDがあれば一貫性を確認
4. 曖昧な要件は明確化のための質問を生成
5. ステークホルダーを特定
...
## 出力場所
- webapp関連: `webapp/docs/design/{feature-name}.md`
- infra関連: `infra/docs/design/{feature-name}.md`

## テンプレート参照
**必ず対応するディレクトリの `template.md` を読み込んで、そのフォーマットに従って作成してください。**

- webapp: `webapp/docs/design/template.md`
- infra: `infra/docs/design/template.md`

## 作成プロセス

1. まず対応する `template.md` を Read ツールで読み込む
2. 関連PRDを確認
3. テンプレートの構造に従って内容を記入
4. 技術的な代替案を検討
5. ADRが必要な場合は提案
...
## 出力場所
- webapp関連: `webapp/docs/adr/ADR-{number}-{title}.md`
- infra関連: `infra/docs/adr/ADR-{number}-{title}.md`

## テンプレート参照
**必ず対応するディレクトリの `template.md` を読み込んで、そのフォーマットに従って作成してください。**

- webapp: `webapp/docs/adr/template.md`
- infra: `infra/docs/adr/template.md`

## ADR番号の決定
1. 対象の `docs/adr/` ディレクトリを確認
2. 最新のADR番号を取得
3. +1 した番号を使用
...
## 出力場所
- webapp関連: `webapp/docs/plan/{feature-name}-plan.md`
- infra関連: `infra/docs/plan/{feature-name}-plan.md`

## テンプレート参照
**必ず対応するディレクトリの `template.md` を読み込んで、そのフォーマットに従って作成してください。**

- webapp: `webapp/docs/plan/template.md`
- infra: `infra/docs/plan/template.md`
